...
describe("React Testing Library UTV", () => {
  ...
  test("рендеры h1, btn, inp(+снимок)", () => {
    ...
    render(<App />);
    // ^ ч/з спец.объ.`экран`.мтд. получ.эл. (по тексту, роли, тегу, стилю, props, т.д. и т.п.)
  ...
  test("EVENT click", async () => {
    ...
    // ожидается отсутств.эл.
    expect(screen.queryByTestId("toggle-elem")).toBeNull();
    // ^ ч/з спец.объ.`огненное событие`.мтд. получ.событие эл. (по клику, измен., копир., мышь, т.д. и т.п.)
    // клику на toggle-btn
    fireEvent.click(btn);
